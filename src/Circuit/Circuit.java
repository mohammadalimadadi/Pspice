...
public class Circuit {
    ...
    public void setNodeArray(){
        ...
        Node[] n=new Node[num];
        System.arraycopy(node,0,n,0,num);
        setNode(n);
    }
    ...
    public void updatingBranches(double time){
        for(int i=0 ; i < branch.length ; i++ ){
            branch[i].setTime(time);
            branch[i].setCurrent(tick,dv,di);
            branch[i].setCurrent_p(tick,dv,di);
        }
    }
    ...
    public ArrayList<LinkedList<Node>> settingUnions() {
        ...
        while (queue.size()<node.length){//node.length-1
            for (Node node : queue) {
            ...
            Iterator<Node> q=queue.iterator();
            while (q.hasNext()){
                Iterator<Node> q1=q.next().getNeighbor().iterator();
                while (q1.hasNext()){
                    Node node_o=q1.next();
                    if((!queue.contains(node_o))&&(node_o.getAdded()==false)){
                        queue.add(node_o);
                        q=queue.iterator();
                        node_o.setAdded(true);}
                }
            }
        }
        ...
        this.unions=new LinkedList[unions.size()];
        for(int i=0;i<=unions.size()-1;i++){
            this.unions[i]=unions.get(i);
        }
        return unions;
    }
    ...
    public void updatingNodes() {
        ...
        for (int i = 0; i <= unions.length - 1; i++) {
            ...
            if(flag==0) {
                ...
                for (Branch branch : neighborbranchs) {
                    ...
                    for (Node node : unions[i]) {
                        if (node.equals(branch.getNode1())) {
                            //  System.out.println(branch.getCurrent());
                            node.setCurrent_n(node.getCurrent_n() + branch.getCurrent());
                            //    node.setCurrent_p(node.getCurrent_p() + branch.getCurrent_p());
                            //if (unions[i].contains(branch.getNode2())) {
                            //    branch.getNode2().setCurrent_n(node.getCurrent_n() + branch.getCurrent());
                            ...
                            //branch.getNode1().setCurrent_n(node.getCurrent_n() + branch.getCurrent());
                ...
                double current = 0;
                for(Node node: unions[i])
                    current+=node.getCurrent_n();

                unions[i].element().setVoltage(unions[i].element().getVoltage() + dv);
                            ...
                            if (queue.contains(node)) {
                                for (Branch branch : node.getAttachments()) {
                                    if ((branch.getNode1().equals(node_help)) || (branch.getNode2().equals(node_help))) {
                                        if (node.equals(branch.getNode1())) {
                                            branch.setVoltage();
                                            node.setVoltage(node_help.getVoltage() + branch.getVoltage());
                                            //  System.out.println("yes++++++++"+branch.getVoltage());
                                            node.setVoltage_p(node_help.getVoltage_p() + branch.getVoltage());
                                        } else if (node.equals(branch.getNode2())) {
                                            branch.setVoltage();
                                            node.setVoltage(node_help.getVoltage() - branch.getVoltage());
                ...
                for (Branch branch : neighborbranchs) {
                    for (Node node : unions[i]) {
                        if (node.equals(branch.getNode1())) {
                            //  System.out.println(branch.getCurrent());
                            // branch.setCurrent(tick,dv,di);
                            node.setCurrent_n(node.getCurrent_n() + Math.abs(branch.getCurrent()));
                            //    node.setCurrent_p(node.getCurrent_p() + branch.getCurrent_p());
                            //if (unions[i].contains(branch.getNode2())) {
                            //    branch.getNode2().setCurrent_n(node.getCurrent_n() + branch.getCurrent());
                            //  branch.getNode2().setCurrent_p(node.getCurrent_p() + branch.getCurrent_p());
                ...
                // unions[i].element().se/tVoltage_p(unions[i].element().getVoltage()+dv);
                /*    for (Node node : unions[i]) {
                        node.setCurrent((node.getCurrent_n() + node.getCurrent_p()) / 2);
                    }*/

                //  System.out.println(unions[i].element().getCurrent_n());
                //System.out.println(i);
                // System.out.println("yes"+unions[1].element().getCurrent_n());
                // System.out.println("yes"+current);
                double current_n = 0;
                for(Node node: unions[i])
                    current_n+=node.getCurrent_n();

                unions[i].element().setVoltage(unions[i].element().getVoltage() + (this.dv * (current - current_n) / this.di));
                unions[i].element().setVoltage(unions[i].element().getVoltage() - dv);
                                ...
                                for (Branch branch : node.getAttachments()) {
                                    if ((branch.getNode1().equals(node_help)) || (branch.getNode2().equals(node_help))) {
                                        if (node.equals(branch.getNode1())) {
                                            ...
                                            node.setVoltage(node_help.getVoltage() + branch.getVoltage());
                                            node.setVoltage_p(node_help.getVoltage_p() + branch.getVoltage());
                                            System.out.println("*" + node_help.getVoltage());
                                            System.out.println("+" + node.getVoltage());
                                        } else if (node.equals(branch.getNode2())) {
                                            branch.setVoltage();
                                            node.setVoltage(node_help.getVoltage() - branch.getVoltage());
                ...
                while (queue.size()!=0) {
                    node_help = queue.poll();
                    // System.out.println(node_help.getnumber());
                    Iterator<Node> node1 = node_help.getNeighbor().listIterator();
                            ...
                            if (queue.contains(node)) {
                                for (Branch branch : node.getAttachments()) {
                                    if ((branch.getNode1().equals(node_help)) || (branch.getNode2().equals(node_help))) {
                                        if (node.equals(branch.getNode1())) {
                                            node.setVoltage(node_help.getVoltage() + branch.getVoltage());
                                            // System.out.println("p" + node.getnumber());
                ...
                for (Branch branch : neighborbranchs) {
                    for (Node node : unions[i]) {
                        ...
                        // System.out.println(branch.getCurrent_p());
                    }
                }
                for (Node node : unions[i]) {
                    node.setCurrent((node.getCurrent_n() + node.getCurrent_p()) / 2);
                    //   System.out.println(node.getCurrent_p());
                }
            }
    ...
    public void printingResult(){
        for(int i=0;i<=node.length-1;i++){
            System.out.println("Node"+node[i].getnumber()+":    "+"V = "+node[i].getVoltage()+" , "+"I = "+node[i].getCurrent());
        }
        System.out.println();

        for(int i=0;i<=branch.length-1;i++){
            branch[i].setVoltage();
            System.out.println("Type:    "+branch[i].getName()+" , "+"V = "+branch[i].getVoltage()+" , "+"I = "+branch[i].getCurrent());
        }
        System.out.println();
    }
