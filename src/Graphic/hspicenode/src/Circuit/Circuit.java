...
public void setDv(double dv){
    ...
    public void setNodeArray(){
        ...
        node_h=new Node[num];
        System.arraycopy(node,0,node_h,0,num);
        node=new Node[num];
        System.arraycopy(node_h,0,node,0,num);
    }
   ...
   public Circuit(int noNodes, int noBranches,double tick,double dv) {
    ...
    public void updatingNodes() {
       ...
       //  System.out.println(unions[i].element().getCurrent_n());
                    unions[i].element().setVoltage(unions[i].element().getVoltage() - (this.dv *( unions[i].element().getCurrent_n())));
       // if((unions[i].element().getVoltage()-unions[i].element().getVoltage_p())<=Math.abs(0.000001)
    ...
    public ArrayList<LinkedList<Node>> settingUnions() {
        ...
        while (queue.size()<node.length-1){
            ...
            while (q.hasNext()){
                Iterator<Node> q1=queue.iterator();
                while (q1.hasNext()){
                    //one operation is added
                    if((!queue.contains(q1.next()))&&(q1.next().getAdded()==false)){
                    queue.add(q1.next());
                    q1.next().setAdded(true);}
                }
            }
        }
        for (Node node : queue) {
