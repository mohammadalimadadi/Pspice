...
public class Netlist {
    ...
    public Circuit readingFile(File S) {
        ...
        for(int i=0;i<=circuit.getNode().length-1;i++)
        {
            for (int j=0;j<=circuit.getNode()[i].getAttachments().size()-1;j++)
            {
        ...
        for (int i=0;i<=circuit.getBranches().size()-1;i++)
        {
